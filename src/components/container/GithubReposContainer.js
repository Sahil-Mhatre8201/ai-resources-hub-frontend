...
const GithubReposContainer = () => {
  ...
  const fetchRepos = async (pageNum) => {
    ...
    try {
      const res = await axios.get(
        `https://ai-resources-hub-backend.onrender.com/search-ai-repos?q=artificial intelligence&max_results=${maxResults}&page=${pageNum}`
      );
  ...
  return (
    <div className="w-full px-3 sm:px-4 md:px-6 py-4 sm:py-6 md:py-8 space-y-4 sm:space-y-6">
      <h2 className="text-xl sm:text-2xl md:text-3xl font-bold">Trending GitHub Repositories</h2>
